...
export default function CalendarGrid({dates, onSelectDate, data, selectedDay, currentDate, initialSelectedDayIndex}){
  ...
  const datesArray = dates.map((date, i) => {
    ...
    return (
      <span 
        onClick={() => isAvailable(date) && changeSelectedIndex(i)}
        data-date={date}
        key={date}
        className={`day${ isToday ? " today" : ""} ${ isAvailable(date) ? "available" : "notAvailable"} ${dateString === formatDate(dates[initialSelectedDayIndex]) ? "selected-day" : ""}`}
